...
- El **perfil del usuario/entidad** (rol, ente, normas aplicables, ejercicio, carpetas locales) lo
  completa el skill `cold-start-interview`, que reemplaza los `[PLACEHOLDER]` de la sección 1.
- Mientras haya `[PLACEHOLDER]` sin completar, Claude debe **ofrecer correr
  `/registracion-estados-contables:cold-start-interview`** antes de producir trabajo sustantivo.
